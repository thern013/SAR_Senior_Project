...
export class WaterfallComponent implements OnInit, AfterViewInit {
  ...
  heights: any 
  mesh: any;
  time_samples = 128; // X resolution
  n_vertices = (this.frequency_samples + 0) * (this.time_samples + 0);
  xsegments = this.time_samples;
  ysegments = this.frequency_samples;
  xsize = 25;
  ysize = 25;
  xhalfSize = this.xsize / 2;
  yhalfSize = this.ysize / 2;
  xsegmentSize = this.xsize / this.xsegments;
  ...
  ngAfterViewInit(): void {
    ...
    // this.radarFeedService.getSampleRate().subscribe(sampleRate => {
    //   this.frequency_samples = sampleRate;
    // })
  }
  ...
  private initThreeJs(): void {
    ...
    for (let i = 0; i <= this.xsegments; i ++ ) {
      let x = ( i * this.xsegmentSize ) - this.xhalfSize; //midpoint of mesh is 0,0
      for ( let j = 0; j <= this.ysegments; j ++ ) {
        let y = (j * this.ysegmentSize) - this.yhalfSize;
        vertices.push( x, y, 0);
        this.heights.push(10); // make them flat again
      }
    }
    ...
    geometry.setAttribute('displacement', new THREE.Uint16BufferAttribute(this.heights,1));

    // generate indices (data for element buffer)
    for (let i = 0; i < this.xsegments; i++) {
      for (let j = 0; j < this.ysegments; j++) {
  ...
  update_geometry() {
    ...
    // this.heights.copyWithin(0, start_val, this.n_vertices+1);  //moves to the write
    // this.heights.set(this.DATA, end_val-start_val);  
    this.mesh.geometry.setAttribute('displacement', new THREE.Float32BufferAttribute(this.heights, 1));
  }

  update_radar_feed(amplitude: any) {
    // set the radar area of effect of the scene.  The visible scene is time_samples + 1 * frequency_samples + 1
    // the offest will respresent the current position of the radar
    let xOffset = (this.time_samples) / 2 - 8; // max is the (time samples) - xaoe * 2
    let yOffest = (this.frequency_samples) / 2 - 8; // max is the (frequency samples) - yaoe * 2

    // the aoe will determine the size of the radar.  The smaller values will result in a greater resolution
    let xAoe = this.time_samples / 16;
    let yAoe = this.frequency_samples / 16;

    // origins reference from the bottom left of the scene
    let yOrigin = 0 + yOffest;
    let yEnd = yOrigin + (yAoe * 2);
    
    let xOrigin = 0 + xOffset;
    let xEnd = xOrigin + (xAoe * 2)

    // this controls the origin placement of the aoe along the x axis and y axis
    for (let i = xOrigin; i < xEnd; i++ ) {
      let xIter = (this.frequency_samples + 1) * i;
      this.heights.fill(amplitude, yOrigin + xIter, yEnd + xIter);
    }

    this.heights = new Uint8Array(this.heights);
    this.mesh.geometry.setAttribute('displacement', new THREE.Uint16BufferAttribute(this.heights,1));
    this.renderer.render( this.scene, this.camera );
  }
}
